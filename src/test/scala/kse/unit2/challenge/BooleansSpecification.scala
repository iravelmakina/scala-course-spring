...
object AxiomsSpecification extends Properties("Axioms"):
  ...
  property("a → (b → a) ≡ True") = forAll: (a: Boolean, b: Boolean) =>
    (a → (b → a)) == True

  property("(a → (b → c)) → ((a → b) → (a → c)) ≡ True") = forAll: (a: Boolean, b: Boolean, c: Boolean) =>
    ((a → (b → c)) → ((a → b) → (a → c))) == True

  property("(a ∧ b) → a ≡ True") = forAll: (a: Boolean, b: Boolean) =>
    ((a ∧ b) → a) == True

  property("(a ∧ b) → b ≡ True") = forAll: (a: Boolean, b: Boolean) =>
    ((a ∧ b) → b) == True

  property("a → (b → (a ∧ b)) ≡ True") = forAll: (a: Boolean, b: Boolean) =>
    (a → (b → (a ∧ b))) == True

  property("a → (a ∨ b) ≡ True") = forAll: (a: Boolean, b: Boolean) =>
    (a → (a ∨ b)) == True

  property("b → (a ∨ b) ≡ True") = forAll: (a: Boolean, b: Boolean) =>
    (b → (a ∨ b)) == True

  property("(a → c) → ((b → c) → ((a ∨ b) → c)) ≡ True") = forAll: (a: Boolean, b: Boolean, c: Boolean) =>
    ((a → c) → ((b → c) → ((a ∨ b) → c))) == True

  property("!a → (a → b) ≡ True") = forAll: (a: Boolean, b: Boolean) =>
    (!a → (a → b)) == True

  property("(a → b) → ((a → !b) → !a) ≡ True") = forAll: (a: Boolean, b: Boolean) =>
    ((a → b) → ((a → !b) → !a)) == True

  property("a ∨ !a ≡ True ≡ True") = forAll: (a: Boolean) =>
    (a ∨ !a) == True
